...
\subsubsection{Easily extensible to other languages}

When describing the rule-based approaches to machine translation, I pointed out that for most approaches one needs a specific ruleset for each language pair. The Interlingua is an attempt to solve this problem by adding a language in between, so that only a ruleset for every language-Interlingua pair is needed. Statistical and memory-based approaches solve this problem in an even more elegant way: the only thing needed are computational power, time, and a lot of data. As digital texts are available in large quantities for the more important languages of the world, adding more languages to Colibri should not be that problematic. And also less important languages, even languages of which the grammar has not yet been investigated, can be added easily, as long as there is enough data to train the classifiers (but also see the first two weaknesses). Obviously, the opposite is true for the rule-based approach.

\subsubsection{Improving communication between the world's communities}

\subsection{Weaknesses}

\subsubsection{Memory based learning and the quantity/quality of the data}

%Lack of control

\subsubsection{Existension possibilities to non-European languages?}

\subsubsection{Machine translation and the societal status of translators}

I'd like to close this section with a more general critique of machine translation. Machine translation is far from perfect. Colibri of course is an interesting attempt to bring the technology closer to the 'perfect'-end, it will almost certainly not result in an application which is able to replace human translators. However, when non-perfect software like this is made available to the public, ignorant users might treat it as if it was perfect software, thereby turning off their own intelligence, as it were. A similar critisism can be heard in the field of spell checkers: 

%Overzicht kritiek, zie Wikipedia

And although giving a detailed analysis of the validity of these arguments is beyond the scope of this paper, I'd like to point out that something similar already seems to be happening: since the release of Google Translate, various examples of websites, street signs, restaurant menus and other texts translated by the service can be found all over the internet. The authors of the text, apparently not skilled in the target language, have relied on machine translation only and didn't check the quality translation in any other way, and have put this incorrect (and often funny) language in a place for the world to see.\\\indent
An to make the problem even more complicated, there probably is no such thing as a 'correct' translation, in contrast to what the previous paragraph may have suggested. That is, if you ask n translators to translate a long text in language A to language B, n different translations will result. Translation is a problem for which no clearly defined answer exists. Instead, what one considers a good translation is largely a matter of taste, beliefs, cultural preference, etc., making translation much closer to an art. Viewing translation as something a computer can do might be a huge underestimation of the task, and giving the ignorant user this impression could be argued to be very misleading.
